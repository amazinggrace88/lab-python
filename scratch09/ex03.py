...
2) csv 파일을 csv.reader 를 사용해서 파일의 내용을 리스트로 변환
 각 주식 종목에 주식 가격 평균을 계산해서 출력

3) csv 파일을 csv.DictReader 를 사용해서 파일의 내용을 리스트로 변환
...
# 파일을 쓰기 모드로 열기  -->  파일을 새로 만들어서 쓰기 모드로 여는 거
...
# 2) csv 파일을 csv.reader 를 사용해서 파일의 내용을 리스트로 변환 / 각 주식 종목에 주식 가격 평균을 계산해서 출력
# 파일을 읽기
...
msft_prices = [float(item[2]) for item in df
               if item[1] == 'MSFT']
print(f'msft_prices = {msft_prices}, msft_avg = {sum(msft_prices)/len(msft_prices)}')
...
# 3 csv 파일을 csv.DictReader 를 사용해서 파일의 내용을 리스트로 변환. 각 주식 종목의 주식 가격 평균을 계산해서 출력 (컬럼 이름을 생성해주세요!)
with open('stock_price.csv', mode='r', encoding='UTF-8') as f:
    # csv Dict Reader 객체를 생성
    # csv 에 칼럼 이름이 없는 경우, 생성자에 fieldnames 파라미터에 컬럼 이름 값들을 만들어서 전달~
