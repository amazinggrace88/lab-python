...
students = pd.DataFrame([
    ...
    [14, '이재경', 'M'],
    [24, '조지원', 'F']
], columns = ['번호', '이름', '성별'])  # columns라는 property를 사용하여 칼럼의 이름 변경
# students.columns = ['번호', '이름', '성별']  # 컬럼의 이름 부여하는 함수 .columns
print(students)  # 2차원 리스트 -> df 로 출력됨 컬럼의 이름이 없으면 index가 자동 부여된다.
