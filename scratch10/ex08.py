...
- pip install beautifulsoup4 (순서3)
...
with open('web01.html', mode='r', encoding='UTF-8') as f:
    ...
    # HTML 요소들 중에서 tag 이름으로 h1 요소를 찾음 (방법 (1) (2))
    # (1) find('태그이름')
    h1 = soup.find('h1')
    ...
    # find 의 동작 원리 : for 문처럼 찾으면 바로 리턴 (처음부터 분석 하다가 가장 처음에 만나는 요소를 리턴)
    print(soup.find('a'))  # anchor a tag 를 하나 밖에 찾을 수 없다.
    print(soup.a)  # 같은 결과

    # findall : HTML 문서 전체에서 찾은 모든 해당 요소들의 리스트를 리턴함.
    print(soup.find_all('a'))
    print(soup('a'))  # 같은 결과 : soup('태그이름')은 soup.find_all('태그이름')과 동일

    # HTML 문서의 모든 링크에서 링크 주소(href)만 추출해서 출력
    for link in soup('a'):
        # HTMLelement.get('attr 이름') - attr 의 값을 구함.
        print(link.get('href'))
