...
print(y1)
plt.plot(x, y1, 'go--', label='example1')  # x축의 scale 과 y축의 scale 이 같지 않지만, plt.plot 이 자동 조정해준다.
# r: red + : 점선 => ctrl+q 해서 문서에서 찾아보고 조합한다.
# go-- : 선 green , o 마커, -- dashed line style
# 여러개 선 그래프 만들기 -> plt.show() 하기 전까지 여러가지의 plt.plot 을 둔다.
y2 = [2 ** x for x in range(9, -1, -1)]
print(y2)
plt.plot(x, y2, 'r--', label='example2')
# vector 합 using list comprehension
y3 = [x + y for x, y in zip(y1, y2)]  # 중요*****list2개를 zip 하여 dict 를 만들기도 함.
print(y3)
